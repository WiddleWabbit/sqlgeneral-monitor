...
def getUser(ln, configuration):
    ...
    for db in configuration:
        ...
        if configuration[db][CONFIG.Database.value] in ln:
...
#               ################################
    ...
    if os.path.exists(options.location) and os.path.isfile(options.location):
        ...
        if options.debug_line and options.debug_line > counter:
...
#               ####################
        ...
        if options.debug_line is not None:

            # Convert the number given to the array index of the line        
            test_index = options.debug_line - 1
